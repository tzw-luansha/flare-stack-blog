          ...
          <section className="space-y-8">
            ...
            <div className="space-y-4">
              {myLinks.map((link) => (
                <div
                  key={link.id}
                  className="glass-card p-4 md:p-5 flex items-start justify-between gap-4"
                >
                  <div className="space-y-2 min-w-0 flex-1">
                    ...
                    <a
                      ...
                      className="text-[11px] font-mono text-muted-foreground hover:text-accent transition-colors flex items-center gap-1"
                    >
...
function FriendLinkStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`glass-pill text-[9px] uppercase tracking-wider font-mono ${styles[status] || ""}`}
    >
