...
export function FriendLinkCard({ link }: FriendLinkCardProps) {
  ...
  return (
    <a
      ...
      className="glass-card group flex items-start gap-4 p-4 md:p-5 transition-all hover:bg-white/[0.06] dark:hover:bg-white/[0.06]"
    >
      {/* Logo */}
      <div className="shrink-0 w-10 h-10 rounded-xl bg-white/10 dark:bg-white/5 border border-white/20 dark:border-white/5 flex items-center justify-center overflow-hidden">
        {link.logoUrl ? (
          <img
            src={link.logoUrl}
            alt={link.siteName}
